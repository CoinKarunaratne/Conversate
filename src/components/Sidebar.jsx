...
import { setConversation, setNumber } from "../state";
      ...
      try {
        ...
        const filteredUsers = filteredRoom.map((room) => ({
          ...
          lastMessage: room.messages
            ...
            : null,
          users: room.users
            .filter((user) => user !== currentUser.uid)
            .join(", "),
        }));
    ...
    <div className="p-8 overflow-y-scroll">
      {rooms?.map((room, index) => (
        <motion.div
          ...
          key={index}
        >
          <Flex className="mb-8 p-5 rounded-xl shadow-black shadow-md bg-red-100">
            <HStack className="gap-4">
              ...
              <VStack>
                ...
                <p className=" self-start">{room.lastMessage?.text}</p>
              </VStack>
            </HStack>
          </Flex>
        </motion.div>
